...
public class ZookeeperController {
    ...
    private String create(@PathVariable Integer type, @PathVariable String znode, @RequestParam String nodeData){
        znode = "/" + znode;
        try {
    ...
    public String update(@RequestParam String znode,@RequestParam String nodeData){
        znode = "/" + znode;
    ...
    public String find(@RequestParam String znode){
        znode = "/" + znode;
    ...
    public String counter(@RequestBody  String znode){
        SharedCount baseCount = new SharedCount(zkClient.getClient(), znode, 0);
        try {
            baseCount.start();
            //生成线程池
    ...
    public String distributedCount(@RequestBody  String znode) throws Exception {
        DistributedAtomicLong distributedAtomicLong = new DistributedAtomicLong(
                zkClient.getClient(), znode, new RetryNTimes(10, 30));
        //生成线程池
        ExecutorService executor = Executors.newCachedThreadPool();
